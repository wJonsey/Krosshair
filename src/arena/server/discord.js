...
export class DiscordAuth {
  ...
  async finishToken(accessToken, state) {
    ...
    this.states.delete(state);
    if (!at || Date.now() - at > STATE_TTL) throw new Error('That login link expired. Start again from the game.');
    if (typeof accessToken !== 'string' || accessToken.length < 10 || accessToken.length > 200) throw new Error('Discord did not send a login. Try again.');
    const response = await fetch(`${this.api}/oauth2/@me`, { headers: { Authorization: `Bearer ${accessToken}` } });
    if (!response.ok) throw new Error('Discord did not accept that login. Try again.');
    const info = await response.json();
    if (String(info.application?.id) !== this.clientId || !info.user) throw new Error('That login was not issued for Krosshair.');
    let joined = false;
...
// Shown when someone presses the Discord button on a server that has no application ID yet.
export function setupPage(redirect) {
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Krosshair — Discord setup</title><style>body{margin:0;min-height:100vh;display:grid;place-items:center;background:#0a0e12;color:#e6edf1;font:15px/1.7 system-ui,sans-serif}main{max-width:620px;padding:32px}h1{font-size:20px}code{background:#141a20;padding:2px 6px;color:#ffb547;word-break:break-all}a{color:#ffb547}li{margin-bottom:10px}</style></head><body><main><h1>Discord login needs one more step</h1><p>Krosshair signs pilots in with Discord, and this server has not been told which Discord application to use yet.</p><ol><li>Create an application at <a href="https://discord.com/developers/applications" target="_blank" rel="noopener">discord.com/developers/applications</a>.</li><li>Under <b>OAuth2 → Redirects</b>, add <code>${escapeHtml(redirect)}</code></li><li>Copy the <b>Application ID</b> (a long number — it is public, not a secret) into <code>DISCORD_CLIENT_ID</code> in <code>src/arena/shared/constants.js</code>, then restart the server.</li><li>Optional, to add pilots to the Discord server automatically: create the app’s bot, invite it to the server, and put its token in <code>.env</code> as <code>DISCORD_BOT_TOKEN</code>.</li></ol><p><a href="/">Back to Krosshair</a></p></main></body></html>`;
}
