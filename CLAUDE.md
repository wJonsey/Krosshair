...
| Friends, parties | `server/social.js` (the friendship rules), `server/party.js` (`PartyBook`), `handleFriends`/`handleParty` in `multiplayer-server.mjs`, `client/social.js` | Friendship is mutual: a request, then an accept. Parties are in memory and follow their leader into the match. See `HANDOFF.md`. |
...
| Weapon levels | `shared/gunlevels.js` (all the tuning), `ProfileStore.gunXp`/`awardGunXp`/`usableBuild` in `server/profiles.js`, `gunXp` in `room.js` `kill()` | Per pilot, per gun, saved with the profile (guests: memory only). Parts never change the Armoury price. The server pays XP and decides unlocks; the Gunsmith only draws them. See `HANDOFF.md`. |
