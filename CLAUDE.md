...
| Weapon levels | `shared/gunlevels.js` (all the tuning), `ProfileStore.gunXp`/`awardGunXp`/`usableBuild` in `server/profiles.js`, `gunXp` in `room.js` `kill()` | Per pilot, per gun, memory only: a restart resets them. The server pays XP and decides unlocks; the Gunsmith only draws them. See `HANDOFF.md`. |
