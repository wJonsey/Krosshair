...
| Classes | `shared/classes.js`, `saveClasses` in `server/profiles.js`, `buyClass` in `server/room.js`, `classesHtml` in `client/shop.js` | A saved shopping list the armoury buys in one press. It grants nothing: it pays the ordinary price through the ordinary buy path. See `HANDOFF.md`. |
